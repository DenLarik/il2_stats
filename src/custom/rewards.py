...
def guards(player):
    if player.coal_pref == Coalition.Allies and player.squad:
        if player.squad.get_position_by_field() == 1 and (
                player.get_rating_reward_count('guards') != player.squad.num_members or not player.is_rewarded(
            'guards')):
...
def luftwaffe_badge(player):
    if player.coal_pref == Coalition.Axis and player.squad:
        if player.squad.get_position_by_field() == 1 and (
                player.get_rating_reward_count(
                    'luftwaffe_badge') != player.squad.num_members or not player.is_rewarded(
            'luftwaffe_badge')):
            player.delete_rating_reward('guards')
            player.delete_rating_reward('luftwaffe_badge')
...
def medal_for_bravery(sortie):
    if sortie.player.coal_pref == Coalition.Allies or (
            sortie.player.sorties_coal[Coalition.Allies] == sortie.player.sorties_total):
        return sortie.score > 0 and sortie.status == SortieStatus.landed and (
                (sortie.aircraft_status == LifeStatus.damaged and sortie.damage > 40) or (
                sortie.bot_status == BotLifeStatus.wounded and sortie.wound > 25))


# Железный крест 1-го класса - условия как у медали "За отвагу"
def iron_cross_1st_class(sortie):
    if sortie.player.coal_pref == Coalition.Axis or (
            sortie.player.sorties_coal[Coalition.Axis] == sortie.player.sorties_total):
        return sortie.score > 0 and sortie.status == SortieStatus.landed and (
                (sortie.aircraft_status == LifeStatus.damaged and sortie.damage > 40) or (
                sortie.bot_status == BotLifeStatus.wounded and sortie.wound > 25))
...
def gold_star_3rd_ground(player):
    if player.is_top_ground_streak() and not player.is_rewarded(
            'gold_star_3rd_ground') and player.coal_pref == Coalition.Allies:
...
def order_of_lenin(sortie):
    if sortie.player.coal_pref == Coalition.Allies and sortie.player.is_officer():
        if sortie.score > 0 and (sortie.ak_total >= 4 or sortie.tanks_total >= 6):
            return True
        elif sortie.score >= 600 and (sortie.ak_total >= 1 and sortie.gk_total >= 5) and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def knights_cross(sortie):
    if sortie.player.coal_pref == Coalition.Axis and sortie.player.is_officer() and not (
            sortie.player.is_rewarded('knights_cross_leaves') or sortie.player.is_rewarded(
        'knights_cross_leaves_swords') or sortie.player.is_rewarded(
        'knights_cross_leaves_swords_diamonds') or sortie.player.is_rewarded(
        'knights_cross_leaves_swords_diamonds_gold') or sortie.player.is_rewarded(
        'knights_cross_leaves_swords_diamonds_gold_ground')):
        if sortie.score > 0 and (sortie.ak_total >= 4 or sortie.tanks_total >= 6):
            return True
        elif sortie.score >= 600 and (sortie.ak_total >= 1 and sortie.gk_total >= 5) and (
...
def order_of_glory_3rd_class(sortie):
    if (sortie.player.coal_pref == Coalition.Allies or (sortie.player.sorties_coal[
                                                            Coalition.Allies] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and sortie.ak_total >= 1 and sortie.gk_total >= 1 and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def order_of_glory_2nd_class(sortie):
    if (sortie.player.coal_pref == Coalition.Allies or (sortie.player.sorties_coal[
                                                            Coalition.Allies] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and sortie.killboard_pve.get('ship', 0) >= 3 and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def order_of_glory_1st_class(sortie):
    if (sortie.player.coal_pref == Coalition.Allies or (sortie.player.sorties_coal[
                                                            Coalition.Allies] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and (
                sortie.killboard_pve.get('tank_heavy', 0) >= 3 or sortie.killboard_pve.get('tank_medium',
                                                                                           0) >= 6 or sortie.killboard_pve.get(
            'tank_light', 0) >= 8) and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def war_merit_cross_2nd_class(sortie):
    if (sortie.player.coal_pref == Coalition.Axis or (
            sortie.player.sorties_coal[
                Coalition.Axis] == sortie.player.sorties_total)) and not sortie.player.is_officer():
...
def war_merit_cross_1st_class(sortie):
    if (sortie.player.coal_pref == Coalition.Axis or (
            sortie.player.sorties_coal[
                Coalition.Axis] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and sortie.tanks_total >= 4 and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def knights_war_merit_cross(sortie):
    if (sortie.player.coal_pref == Coalition.Axis or (
            sortie.player.sorties_coal[
                Coalition.Axis] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and sortie.gk_total >= 50 and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
...
def knights_war_merit_cross_swords(sortie):
    if (sortie.player.coal_pref == Coalition.Axis or (
            sortie.player.sorties_coal[
                Coalition.Axis] == sortie.player.sorties_total)) and not sortie.player.is_officer():
        if sortie.score > 0 and sortie.tanks_total >= 6 and (
                sortie.aircraft.cls == 'aircraft_heavy' or sortie.aircraft.cls == 'aircraft_medium'):
            return True
